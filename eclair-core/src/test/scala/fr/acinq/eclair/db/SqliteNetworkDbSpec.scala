...
import fr.acinq.eclair.{ShortChannelId, randomKey, randomBytes}
...
class SqliteNetworkDbSpec extends FunSuite {
  ...
  test("add/remove/list channels and channel_updates") {
    ...
    def sig = Crypto.encodeSignature(Crypto.sign(randomBytes(32), randomKey)) :+ 1.toByte
  ...
  test("remove many channels") {
    val sqlite = inmem
    val db = new SqliteNetworkDb(sqlite)
    val sig = Crypto.encodeSignature(Crypto.sign(randomBytes(32), randomKey)) :+ 1.toByte
    val priv = randomKey
